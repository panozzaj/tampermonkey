...
(function() {
    ...
    function extractItemData(itemElement) {
        ...
        if (!authorElement) {
            const bylineElements = itemElement.querySelectorAll('[id^="item-byline-"]');
            if (bylineElements.length > 0) {
                data.author = bylineElements[0].textContent.replace(/^by\s*/i, '').trim();
            }
        } else {
        ...
        formatLinks.forEach(link => {
            ...
            const formatMatch = text.match(/(Hardcover|Paperback|Kindle|Audio CD|Audible Audiobook|Mass Market Paperback|Board book|Library Binding)/i);
        ...
        const itemDetailsArea = itemElement.querySelector('[class*="itemUsedAndNew"]');
        if (itemDetailsArea) {
        ...
        const libraryIndicator = itemElement.querySelector('[class*="library"], [data-library]');
    ...
    function convertToCSV(items) {
        const headers = ['Title', 'Author', 'ASIN', 'List Price', 'Library Available'];
    ...
    async function extractWishlistData(autoScroll = false) {
        ...
        try {
            ...
            if (autoScroll) {
                isScrolling = true;
                alert('Starting auto-scroll to load all items. This may take a while...');
            ...
            if (items.length === 0) {
                alert('Could not extract any item data. The page structure may have changed.');
                return;
            }
